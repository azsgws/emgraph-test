...
def make_pagerank_graph(mml_version, style="dot"):
    cwd = os.getcwd()
    ...
    # pagerankに順位付け
    node2ranking = rank_nodes_with_pagerank(node2pagerank)
    # pagerankの順位をもとにグループ分け
    node2group = grouping_for_ranking(node2ranking)
    # pagerankの値を正規化して，属性値'pagerank'に登録
    node2pagerank = update_pagerank_for_set_node_attribute(node2pagerank)
    # pagerankの値，順位をグラフの属性値として定義する
    nx.set_node_attributes(G, node2ranking)

    nx.set_node_attributes(G, node2pagerank)
    ...
    try:
        os.chdir("graph_attrs")
        with open(style + "graph_" + mml_version + "_pagerank.json", "w") as f:
